...
export default function ProductDetailPage() {
  ...
  const [quantityExceedsStock, setQuantityExceedsStock] = useState(false);
    ...
    if (loginUser === null) {
      ...
      try {
        ...
        if (err instanceof AxiosError && err.response?.status === 400) {
          setQuantityExceedsStock(true);
          setIsAddingToCart(false);

          setTimeout(
            () => {
              setQuantityExceedsStock(false)
            }, 5000
          )
        } else {
          navigate({to: "/error"})
  ...
  const renderAddtoCartBtn = () => {
    if (productDto && productDto.stock > 0) {
      ...
      return (
        <Button className="btn-dark text-white" onClick={handlePutCartItem} style={{width: 180}}>Add to
          cart</Button>
      )
    } else {
      return (
  ...
  // const genreName =
  // const navToGenrePage = ()=>{
  //   navigate({to:"/genre/$genreName"})
  // }
            ...
            <Container className="my-5">
              <div className="d-flex justify-content-center">
                ...
                <div className="ms-4">
                  ...
                  <Link to={"/genre/$genreName"} params={{genreName: productDto.genre.toString()}} style={{ textDecoration: 'none' }}>
                  ...
                  {renderAddtoCartBtn()}
                  {
                    ...
                    <Alert key="warning" variant="warning" className="mt-3 text-center">
                      Exceeds stock!
                    </Alert>
                  }
                </div>
